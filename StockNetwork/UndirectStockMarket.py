...
class UndirectStockMarket(object):
    ...
    def GeneratingCompeleteGraph(self,stock_list,start=None,end=None,folder_name=None,file_name = None):
        ...
        for x in stock_list:
            for y in stock_list:
                if x != y:
                    r = UndirectEdge.Pearson_correlation_coefficient(x, y,start,end)
...
if __name__ == '__main__':
    ...
    for i in range(365):
        ...
        path = s.generate_path(start=start, end=end, folder_name='UndirectStockMarket', base_name='hz300s')
        if os.path.isfile(path):
            print 'get',path
        else:
            s.GeneratingCompeleteGraph(stock_list = stock_list,
                                       end=end,
                                       folder_name='UndirectStockMarket',
                                       file_name = 'hs300s')
